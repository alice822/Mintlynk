...
namespace MintLynk.Web.Areas.Dashboard.Controllers
{
    ...
    public class LinksController : Controller
    {
        ...
        public LinksController(ILogger<LinksController> logger, IUrlShorteningService urlShorteningService, IConfiguration configuration, ILinkStatsService linkStatsService)
        {
            _logger = logger;
        ...
        [Route("app/dashboard/short-links")]
        public async Task<IActionResult> Index(string q, int page = 1, int pageSize = 15)
        {
            ...
            var viewModel = linksData.Where(x=>x.LinkType == (int)SmartLinkType.ShortUrl).ToList();
            ...
            var finalViewModel = pagedLinks.Select(dto =>
            {
                ...
                return viewDto;
            }).Where(x => x.LinkType == (int)SmartLinkType.ShortUrl).ToList();

            return View(finalViewModel);
        }

        [HttpGet]
        [Route("app/dashboard/short-links/create")]
        ...
        [Route("app/dashboard/short-links/{entityId}/detail")]
        public async Task<IActionResult> Detail(string entityId)
        {
            ...
            if (linksData.LinkType != (int)SmartLinkType.ShortUrl)
            {
                _logger.LogWarning($"Short link with EntityId {entityId} is not a short URL.");
                return NotFound();
            }
        ...
        [HttpGet]
        [Route("app/dashboard/short-links/{entityId}/edit")]
        ...
        public IActionResult Edit(string entityId, SmartLinkViewDto model)
        {
            ...
            var redirectUrl = $"{model.EntityId}/detail";
            return Redirect(redirectUrl);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        [Route("app/dashboard/short-links/{entityId}/delete")]
        public IActionResult Delete(string entityId)
        {
            ...
            if (link == null)
            {
                ...
                response.ResponseMessage = "Failed to delete short URL.";
                return Json(response);
            }

            if (link.LinkType != (int)SmartLinkType.ShortUrl)
            {
                response.ResponseType = "error";
                response.ResponseMessage = "Failed to delete short URL.";
                return Json(response);
            ...
            if ((data))
            {
                ...
                response.ResponseMessage = "Short URL deleted successfully.";
                return Json(response);
            }
            else
            {
